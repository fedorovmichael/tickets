...
function editClose() {
	$('html head').find('title').text("Афиша 2018 - купить билет на концерт, театр онлайн в Израиле - BILETY.CO.IL");
...
function fillEditShowHTML(show, arrShowsSeances, arrMedia, arrComments, arrRecommendShows, device) {
	...
	$.each(arrTrs, function(index, value) {
		...
		if(index == 0 && device == "phone"){$(this).remove();} 
	});
	...
	$.each(arrRecommendShows, function(index, value) {
		...
		recommendDivHTML += '<li style="float: left;">' + '<div class="thumbnail">' + '<a href="' + linkShow + '">' + '<img src="http://www.bilety.co.il/images/shows/' + imageName + '" alt="' + name + '" title="' + name + '" class="main-show-image" onerror="handleImageError(this);"/>' + '</a>' + '</div>' + '<div class="caption">' + '<a href="' + linkShow + '" class="a-link-general">' + '<h1 class="main-show-caption">' + name + '</h1>' + '</a>' + '</div>' + '</li>'
	});
